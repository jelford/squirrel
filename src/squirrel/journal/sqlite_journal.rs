...
impl <'a> super::Journal for SqliteJournal {
    fn journal(&mut self, event: Event) -> Result<()> {
        ...
        let before_path = event.before_path.map(|p| format!("{}", p.to_string_lossy()));
...
pub(crate) struct QmErrMapper<T> where T : Iterator<Item=StdResult<Event, RusqlError>> {
    qm: T,
}

impl <'a, T> Iterator for QmErrMapper<T> where T : Iterator<Item=StdResult<Event, RusqlError>>{
